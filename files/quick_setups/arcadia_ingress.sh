...
echo "Waiting for Ingress and Dashboard domain names"
while
  printf "."
  sleep 1
  DASHBOARD_INGRESS=$(kubectl get svc dashboard-nginx-ingress --namespace=nginx-ingress | tr -s " " | cut -d' ' -f4 | grep -v "EXTERNAL-IP")
  INGRESS=$(kubectl get svc nginx-ingress --namespace=nginx-ingress | tr -s " " | cut -d' ' -f4 | grep -v "EXTERNAL-IP")
  [[ "$DASHBOARD_INGRESS" == "<pending>" ]] || [[ "$INGRESS" == "<pending>" ]]
do true; done
echo ""
...
echo "Arcadia Crypto Domain https://$INGRESS"
echo "Nginx Ingress Dashboard https://$DASHBOARD_INGRESS/dashboard.html"
